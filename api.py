...
def api_chat():
    ...
    # Get memory_mode from request, default to "search" if not provided
    memory_mode = data.get("memory_mode", "search")
    format_name = data.get("format")
    
    logger.info(f"Chat request - model: {model}, memory_mode: {memory_mode}, format: {format_name}, user: {conversation_id}")
...
def api_memories():
    """Retrieve or delete memories for a user."""
    ...
    if request.method == 'GET':
        try:
            memories = memory_instance.get_all(user_id=user_id, limit=20)
            return jsonify({"memories": memories})
        except Exception as e:
            logger.error(f"Error retrieving memories: {e}")
            return jsonify({"error": str(e)}), 500
    
    # Handle DELETE request to clear memories
    elif request.method == 'DELETE':
        try:
            # Clear all memories for the specified user
            memory_instance.clear(user_id=user_id)
            return jsonify({"success": True, "message": "Memories cleared."})
        except Exception as e:
